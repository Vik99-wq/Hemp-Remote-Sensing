...
DOMINANT_COLORS = 35
FILEPATH = "kMeansImages/hemp1.jpeg"
NUM_LUMA = 1
threshold_area = 20
...
def generateOriginalImage():
    ...
    plt.imsave('kMeansImages/Original.png', im)
    #plt.show()
    plt.close()
    return im
...
def Colors(centers):
    ...
    plt.savefig('kMeansImages/colorSwatch.png')
    #plt.show()
    plt.close()
    return colors
...
def generateSegmentedImage(new_img):
    segmentedFilepath = 'kMeansImages/segmented.png'
...
def generateBWImage(bw_img, km, colors, original_shape, lightestIndexes):
    BW1Filepath = 'kMeansImages/BW1.png'
...
def imageOpening(img, imageNum, iterations):
    openedFilepath = f'kMeansImages/BW{imageNum}.png'
    img = img.copy()
    opened_img = img
...
def imageClosing(img, imageNum, iterations):
    closedFilepath = f'kMeansImages/BW{imageNum}.png'
    img = img.copy()
    closedImg = img
    for i in range(iterations):
...
def imageDilation(img, imageNum, iterations):
    dilatedFilepath = f'kMeansImages/BW{imageNum}.png'
    img = img.copy()
    dilatedImg = img
    for i in range(iterations):
...
def generateTransparentImage(filepath):
    transparentFilepath = 'kMeansImages/transparent_image.png'
...
def overlayImages(segmentedFilepath, transparentFilepath):
    overlayFilepath = 'kMeansImages/overlayed.png'
    '''
...
def findBlobs(img_filepath):
    cannyFilepath = 'kMeansImages/Canny_Edges_After_Contouring.png'
    contourFilepath = 'kMeansImages/Contours.png'
...
def minimizeBlobs(contours, img):
    cleanedFilepath = 'kMeansImages/cleanedContours.png'
    counter = 0
    for cnt in contours:        
